...
const objectives = [
  {
    ...
    text: 'Develop an automated IoT and ML platform that digitally traces every batch from individual farmer leaf delivery through to final powder output. An ESP32 microcontroller with HX711 load cell sensors automatically captures weights at both the leaf intake and powder output stations, eliminating the manual paper-based recording currently used by factory workers. A Gradient Boosting Regressor model — trained on 2,000 real production records collected from a Badulla region tea factory — predicts the expected yield percentage at the moment a batch is created, before processing begins. The model analyses 12 features including leaf weight, leaf moisture, withering time, fermentation time, drying temperature, drying duration, and live weather data including rainfall, humidity, and temperature fetched automatically from WeatherAPI at batch creation time. The current Sri Lankan tea season is auto-detected from the device date and all five processing parameters are automatically pre-filled with seasonally appropriate defaults to ensure the model always receives contextually correct inputs. When a completed batch deviates from its predicted yield target the cause-specific recommendation engine analyses the actual processing parameters and weather conditions recorded for that batch to identify the root cause and provide specific corrective guidance for the next batch.',
  },
  {
    num: '2',
